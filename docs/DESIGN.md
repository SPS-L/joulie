...
`backup_version` is currently **4**. `BackupSerializer.fromJson` accepts `{3, 4}` and decodes legacy `chargeType = "DC"` to `ChargeType.DC_FAST` via `ChargeTypeJsonAdapter` + `ChargeType.parseLegacy`, so v3 backups in the wild still restore.
