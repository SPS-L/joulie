...
> The SQL below uses snake_case for readability. The actual database — generated by Room from entity declarations in `docs/AGENT_INSTRUCTIONS.md` — uses **camelCase** column names (`costTotal`, `costPerKwh`, `useCount`, `lastUsed`). See §4.2.
...
> **Column-naming note:** the SQL examples in §4.1 use snake_case for readability. The actual database — generated by Room from the entity declarations in `docs/AGENT_INSTRUCTIONS.md` — uses **camelCase** column names (`costTotal`, `costPerKwh`, `useCount`, `lastUsed`, etc.). Migrations must match the entity casing exactly or Room will fail schema validation at startup.
