...
### Baseline profile cadence (TASK-21)

`app/src/main/baseline-prof.txt` is bundled into release APKs and consumed by ART for AOT compilation at install time. It is generated by the sibling `:baselineprofile` module (a `com.android.test` project applying `androidx.baselineprofile`) which drives a macro-benchmark over `:app`'s auto-created `nonMinifiedRelease` variant. Regenerate via `./gradlew :app:generateBaselineProfile` on a connected device or AVD running **API 28+**, then commit the refreshed file. The producer's UIAutomator script lives in `baselineprofile/src/main/java/org/spsl/evtracker/baselineprofile/BaselineProfileGenerator.kt` and covers cold start → wizard → dashboard → FAB → Charts. `StartupBenchmark.kt` measures cold-start `TimeToInitialDisplay` for `CompilationMode.None()` vs `CompilationMode.Partial(BaselineProfileMode.Require)` — run on a device with `./gradlew :baselineprofile:connectedBenchmarkReleaseAndroidTest` when validating profile efficacy.

CI exposes manual regeneration through `.github/workflows/baselineprofile.yml` (workflow_dispatch only — too slow for PR gates). Download the `baseline-prof-txt` artifact and commit it.

**Regenerate the profile when:**
- A major dependency is added or removed (anything that changes the class-loading graph: Hilt, Room, Vico, Drive client).
- Startup ordering changes (`MainActivity`, `MainViewModel`, `EVTrackerApp.onCreate`, the wizard gate).
- Before each `v*` tag push, if any of the above happened since the last regen.

The profile is **not** regenerated on every CI release (`automaticGenerationDuringBuild = false` in `app/build.gradle.kts`) — a constantly-shifting `baseline-prof.txt` diff would dominate release PR reviews and mask real changes.
...
Entities: `Car`, `ChargeEvent`, `CustomLocation`. Current `@Database(version = 9)`. All three entities use `@PrimaryKey val id: Long` and `ChargeEventEntity.carId: Long`. The `@TypeConverters(ChargeTypeConverter::class, ChargeKwhSourceConverter::class)` annotation on `AppDatabase` lets Room round-trip the `chargeType` column between SQLite TEXT and the `ChargeType` enum, and the `kwhSource` column between SQLite TEXT and the `ChargeKwhSource` enum. `ChargeEventEntity` carries optional `socBefore: Double?` and `socAfter: Double?` fields stored as fractions in `0.0..1.0`, a non-null `kwhSource: ChargeKwhSource = ChargeKwhSource.MEASURED` provenance flag, and an optional `gridIntensityGCo2PerKwh: Double?` capturing the Electricity Maps grid mix at save time (TASK-80). `CarEntity` carries an optional `wltpKwhPer100km: Double?` populated from the EV-database autocomplete or by manual edit (TASK-91).
...
- `AutoMigration(from = 8, to = 9)`: adds the nullable `wltpKwhPer100km REAL` column to `cars` for the EV-database autocomplete reference data (TASK-91). Legacy rows migrate to `NULL`. The dialog auto-fills the value when the user picks a model from the dropdown, but all fields remain manually editable so users with rare or modded vehicles aren't locked out.
...
- Backup JSON schema is versioned: current `backup_version = 9` and **must include `custom_locations`** with `label`, `useCount`, `lastUsed`. Bumping any entity requires bumping `backup_version` and updating the authoritative field list in `docs/DESIGN.md §8`. `BackupSerializer.fromJson` accepts `{3, 4, 5, 6, 7, 8, 9}` (legacy `chargeType = "DC"` decodes to `ChargeType.DC_FAST` via `ChargeTypeJsonAdapter`; v3/v4 Int ids narrow into v5+ Long DTO fields automatically via Gson; v3/v4/v5 backups simply leave the new optional `soc_before`/`soc_after` fields at `null`; v3..v6 backups likewise omit `kwh_source`, and `ChargeEventDto.toEntity()` coalesces the absent value to `ChargeKwhSource.MEASURED`; v3..v7 backups omit `grid_intensity_g_co2_per_kwh`, which round-trips to `null`; v3..v8 backups omit `wltp_kwh_per_100km` on `CarDto`, which round-trips to `null` on the nullable entity column).
