...
> **Status:** v1.9.24 tagged (`versionCode = 40`, `versionName = "1.9.24"`). Project rebranded to **Joulie** and the GitHub repo renamed to [`SPS-L/joulie`](https://github.com/SPS-L/joulie); the Kotlin namespace stays `org.spsl.evtracker` and the Room DB filename is unchanged so existing user data survives the upgrade. v1.9.0 adopts the **Joulie Brand Pack v1.0** drop (gradient bolt-J launcher with a `<monochrome>` layer for Android 13+ themed icons; brand-blue splash with white bolt; redesigned wizard welcome and About screens with gradient hero artwork; SPS-Lab badge recoloured from `#FB8C00` to `#FFD54D`; multi-density notification icon; two new colour tokens `joulie_ink_deep #0A1B5E` and `joulie_spark_green #A6F43C`; one new translatable string `wizard_page1_intro_title` with el / tr / ru translations from the pack). The palette is the **Joulie brand**: primary `#0D47FF` (vibrant blue), secondary `#00C2D1` (teal), tertiary `#FFD54D` (yellow, replaces the pre-rebrand "DC orange" `#FB8C00`), accent `#A6F43C` (green). Brand assets live under `docs/branding/` with the canonical [`Joulie_Brand_Guide.pdf`](docs/branding/Joulie_Brand_Guide.pdf) as source of truth, master logos (`joulie_logo_light.png`, `joulie_logo_dark.png`, `joulie_logo_white_bg.png`, `joulie_logo_dark_bg.png`, `joulie_mark_only.png`, `joulie_mark_mono.png`, `brand_palette.png`), editable SVG sources under `svg/`, regeneration pipelines under `scripts/`, and Play Store hi-res icon + light / dark feature graphics under `play-store/`. Read [`docs/branding/README.md`](docs/branding/README.md) before changing any brand surface. **Voice rules** (Brand Guide section 1): plain English; never use the em-dash in prose; prose retrofitted across all 9 user-facing and contributor docs in the v1.9.0 drop. JVM unit-test count: **433**. Instrumented suite compiles via `:app:assembleDebugAndroidTest` (running requires an emulator); Drive backup smoke per `docs/GOOGLE_CLOUD_SETUP.md` needs a Google account allow-listed on the OAuth consent screen. Release signing is wired through a gitignored `keystore.properties`; `.github/workflows/release.yml` builds, signs, verifies, and publishes the APK to a GitHub Release on every `v*` tag push. **GitHub Pages landing page** lives at [sps-l.github.io/joulie](https://sps-l.github.io/joulie/), built and deployed by `.github/workflows/pages.yml` (Pages source = *GitHub Actions*). The privacy page at [/privacy](https://sps-l.github.io/joulie/privacy) is rendered from `PRIVACY.md` at deploy time via pandoc using the `docs/_privacy-template.html` wrapper, so edits to the markdown propagate to the live URL within ~30s.

**Recent post-v1 refactors (load-bearing context for ongoing work). Older entries live in `git log` and `docs/BACKLOG.md`:**

- **TASK-59** (2026-05-06), green up the 10 nightly instrumented failures unmasked once TASK-58 let the suite run to completion. Three About tests needed `scrollTo()` because the license / disclaimer / acknowledgment cards sit below the 240 dp hero on smaller emulators (`isDisplayed()` and the click 90%-visibility constraint require the view to actually be on screen); `spsLabAcknowledgment_isVisible` additionally needed `withId(R.id.about_acknowledgment_lab)` because three TextViews contain "SPS-Lab" (acknowledgment + license body + disclaimer body). One Charts class-level init error: three `@Test fun X() = runBlocking { ... }` methods inferred non-Unit return types from the trailing `.use { ... }` block, which JUnit 4 rejects with `Method X should be void` — wrapped in block bodies. Two ChargeEdit a11y violations: the `MaterialToolbar` nav button had no `contentDescription` (`SpeakableTextPresentCheck` from TASK-18 step 6); added `app:navigationContentDescription="@string/common_navigate_back"` and a new translatable `common_navigate_back` string in en / el / tr / ru. One ManageLocations DAO race: the test asserted `withText("Office")` immediately after `launchFragmentInHiltContainer`, before the seeded row had propagated through `Flow → ViewModel.StateFlow → RecyclerView`; mirrored the second test's `withTimeout(10_000) { observeAll().first { ... } }` poll. Two Settings issues: `row_reset_all` is below the scroll fold (`scrollTo()` before click), and `TestNavHostController(...).apply { setGraph(...) }` triggers `LifecycleRegistry.addObserver` which lifecycle 2.7+ enforces on the main thread (wrapped in `InstrumentationRegistry.getInstrumentation().runOnMainSync`). One Wizard a11y: the 24 dp `TabLayout` indicator dots are clickable by default, failing `TouchTargetSizeCheck`; marked them `isClickable = false` / `isFocusable = false` after `TabLayoutMediator.attach()` (the wizard navigates via Back / Next buttons and `wizardPager.isUserInputEnabled = false` already disables swipe — the dots are decorative). Ancillary brand-pack wiring: `joulie_ink_deep #0A1B5E` is now the About `about_app_name` wordmark `textColor` per the Brand Guide §3 "wordmark colour on light surfaces" scope, with a `values-night/` override mapping to `#B7C4FF` (the M3 inverse-primary tone) so dark-mode wordmark stays legible. `joulie_spark_green` is a kept brand-palette token, absorbed into `lint-baseline.xml` because the brand intent is "fresh affordance / accent strip" rather than body text and a foreground `#A6F43C`-on-`#FDFCFF` text application would fail WCAG AA contrast; mirrors the `chart_4` precedent (same `#A6F43C` value, also baselined for the same reason).
- **TASK-58** (2026-05-06), Hilt-aware host activity for instrumented fragment tests. Every nightly run since the API 26 + API 35 matrix started firing on 2026-05-02 had been red because seven fragment-scenario tests called `androidx.fragment.app.testing.launchFragmentInContainer` against `@AndroidEntryPoint` fragments, hitting Hilt's runtime check on the empty host activity and crashing the instrumentation process before 19 other tests ran. Fix adds `HiltTestActivity` (`@AndroidEntryPoint AppCompatActivity` in `app/src/debug/java/`, declared in a new `app/src/debug/AndroidManifest.xml` so it ships with the debug variant of the *target* APK, mirroring the TASK-50 sub-fix A precedent for `EmptyFragmentActivity`'s manifest entry) plus `org.spsl.evtracker.testing.launchFragmentInHiltContainer` with a thin `HiltFragmentScenario<T>` wrapper that mirrors `FragmentScenario`'s surface (`moveToState`, `recreate`, `onFragment`, `AutoCloseable.use`) so the migration across `AboutFragmentTest`, `ChargeEditFragmentTest`, `ChartsFragmentTest`, `ManageLocationsFragmentTest`, `SettingsFragmentTest`, `SettingsBackupControlsTest`, `SettingsDriveSwitchEntryTest` was a one-line import + identifier swap. Use `launchFragmentInHiltContainer` for any new `@AndroidEntryPoint`-fragment test; `launchFragmentInContainer` is reserved for fragments that don't pull a Hilt graph (none today).
...
scripts/run-instrumented.sh                    # local instrumented suite via ~/Android/Sdk emulator (TASK-69)
```

Requires `ANDROID_HOME` set and Build Tools 35.

### Local instrumented runs (TASK-69, merged 2026-05-06)

`scripts/run-instrumented.sh` replaces the manual `nightly-instrumented.yml` `workflow_dispatch` cycle for day-to-day work. It boots the AVD (KVM-accelerated, ~30 s; falls back to software with a warning if `/dev/kvm` is unavailable, ~10 min boot), disables the three animation scales (Espresso requires this — it is what `reactivecircus/android-emulator-runner@v2`'s `disable-animations: true` does in CI), then runs `:app:connectedDebugAndroidTest`.
...
One-time setup is documented in the script header (sdkmanager / qemu-kvm / avdmanager). The default AVD name is `joulie-test` (override with `JOULIE_AVD=...`). The CI `disable-animations: true` flag is the only behavioural difference between local and CI runs — without `settings put global *_animation_scale 0`, `swipe_showsSnackbar_undo_restoresRow` deterministically fails locally with Espresso's "Animations or transitions are enabled on the target device" message even though the test logic is correct.
